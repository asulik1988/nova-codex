      ...
      while (turnInput.length > 0) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            // Transform the model name if using bedrock Access Gateway
            const modelToUse = this.model;

            const providerKey = this.config.provider?.toLowerCase() || "openai";
            const providerCaps = providerCapabilities[providerKey] || providerCapabilities["default"]

            const responseCall = providerCaps.supportsResponses
              ? (params: ResponseCreateParams) => {
                  console.log('ADAM - DEBUG ACTUAL API REQUEST (responses):', JSON.stringify(params, null, 2));
                  return this.oai.responses.create(params);
                }
              : (params: ResponseCreateParams) => {
                  console.log('ADAM - DEBUG ACTUAL API REQUEST (chat):', JSON.stringify(params, null, 2));
      ...
      if (isNetworkOrServerError) {
        try {
          const msgText =
           ...
           console.error("ADAM DELETE ME -Full error details:", err);
          this.onItem({
            id: `error-${Date.now()}`,
            type: "message",
            role: "system",
